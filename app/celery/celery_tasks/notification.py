...
def send_mass_message(text: str, user_ids: list[int], notification_id: int):
    logger.info(
        f"Starting send_mass_message: notification_id={notification_id}, user_ids={user_ids}"
    )
    success = 0
    blocked = 0
    failed = 0
    ...
    notification_service = NotificationService(
        session_pool=session_pool, redis=redis, config=config
    )
    if success > 0:
        logger.info(
            f"Attempting to update notification {notification_id} as sent (success={success})"
        )
        result = asyncio.run(
            notification_service.update(notification_id, sent_at=datetime_now(), status="Send")
        )
        logger.info(f"Update result for notification {notification_id} (sent): {result}")
    if failed > 0:
        logger.info(
            f"Attempting to update notification {notification_id} as error (failed={failed})"
        )
    ...
    logger.info(
        f"Result: success={success}, blocked={blocked}, failed={failed}, total={len(user_ids)}"
    )
